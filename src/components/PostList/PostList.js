...
const PostList = ({ history, match, location, props }) => {
	...
	useEffect(() => {
		if (wordPressId) {
			const requestPosts = fetch(
				`${config.API_ENDPOINT}/posts?board=${wordPressId}`
			).then((res) => {
				return res.json();
			});
			const requestStyles = fetch(
				`${config.API_ENDPOINT}/styles?board=${wordPressId}`
			).then((res) => {
				return res.json();
			});
			Promise.all([requestPosts, requestStyles])
				.then((res) => {
					setPosts(res[0]);
					setStyles(res[1]);
				})
			...
			const requestPosts = fetch(
				`${config.API_ENDPOINT}/posts?board=${currentBoard.id}`
			).then((res) => {
				return res.json();
			});
			const requestStyles = fetch(
				`${config.API_ENDPOINT}/styles?board=${currentBoard.id}`
			).then((res) => {
				return res.json();
			});
			Promise.all([requestPosts, requestStyles])
			...
			const requestPosts = fetch(
				`${config.API_ENDPOINT}/posts?board=${params.board_id}`
			).then((res) => {
				return res.json();
			});
			const requestStyles = fetch(
				`${config.API_ENDPOINT}/styles?board=${params.board_id}`
			).then((res) => {
				return res.json();
			});
			Promise.all([requestPosts, requestStyles])
