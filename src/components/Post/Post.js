...
export default class Post extends React.Component {
  ...
  deletePost = (e) => {
    ...
    if (answer) {
      PostApiService.deleteStyle(this.props.post.id).then((res) => {
        PostApiService.deletePost(this.props.post.id).then((response) => {
  ...
  render() {
    ...
    if (TokenService.hasAuthToken()) {
      editlink = (
        <div className="post-controls">
          <Link to={`/post/${this.props.post.id}`}>Edit</Link>
    ...
    return (
      <Rnd
        ...
        role="listitem"
        lockAspectRatio={
          this.state.type === "Image" || this.state.type === "Video"
        }
        bounds={"window"}
        ...
        onDragStop={(e, node) => {
          ...
          let heightComponent = this.myRef.current.getSelfElement()
            .offsetHeight;
        ...
        onResizeStop={(e, dir, refToElement, node) => {
          if (!TokenService.hasAuthToken()) {
            return;
          }
